...
import com.upb.agripos.service.ProductCacheService;
...
public class AppJavaFX extends Application {
    ...
    private void showLoginView() {
        // Refresh product cache dari database setiap login
        try {
            ProductCacheService cacheService = ProductCacheService.getInstance();
            cacheService.refreshProductsFromDatabase();
        } catch (Exception e) {
            System.err.println("[NAV] Warning: ProductCacheService refresh failed: " + e.getMessage());
        }
    ...
    private void showPosView() {
        try {
            ...
            if (productService != null) {
                ...
                // Initialize ProductCacheService dengan ProductService
                ProductCacheService.getInstance().setProductService(productService);
                ProductCacheService.getInstance().refreshProductsFromDatabase();
                System.out.println("[NAV] ProductService injected into PosView");
                System.out.println("[NAV] ProductCacheService initialized for synchronization");
            }
