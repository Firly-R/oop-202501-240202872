...
## Ringkasan Sistem
Agri-POS adalah aplikasi kasir yang dirancang khusus untuk toko pertanian. Sistem ini mengintegrasikan manajemen stok produk, proses transaksi penjualan, dan pelaporan keuangan. Aplikasi dibangun menggunakan arsitektur layered (View-Controller-Service-DAO) untuk memastikan kode yang bersih dan mudah dikembangkan.

Fitur Utama:
- Manajemen Produk (CRUD) berbasis PostgreSQL.
- Sistem Transaksi dengan Keranjang Belanja.
- Dukungan Pembayaran Tunai dan E-Wallet menggunakan Strategy Pattern.
- Autentikasi Role-Based (Admin & Kasir).
...
Ruang lingkup WAJIB mengikuti **Functional Requirements (FR) Bab 6** berikut:
