...
3. Pada sistem Agri-POS, polymorphism diterapkan melalui method getInfo() pada kelas Produk yang dioverride oleh Benih, Pupuk,  AlatPertanian, dan ObatHama, sehingga tiap objek menampilkan informasi sesuai jenis produknya.
...
2. Membuat Kelas Subclass (Benih, Pupuk, AlatPertanian)
...
Buat kelas utama MainPolymorphism yang berisi array Produk[] dengan elemen Benih, Pupuk, dan AlatPertanian.
...
3. Hasil praktikum menunjukkan bahwa dynamic binding pada Java bekerja dengan baik, di mana method getInfo() dijalankan sesuai tipe objek aktual (Benih, Pupuk, AlatPertanian, ObatHama).
---
...
3. 3. Berikan contoh kasus polymorphism dalam sistem POS selain produk pertanian.
   **Jawaban:Contohnya pada sistem toko elektronik (E-POS), terdapat superclass Barang dengan method getInfo(). Subclass-nya seperti Televisi, Laptop, dan Smartphone masing-masing meng-override getInfo() untuk menampilkan spesifikasi sesuai jenis produk. Dengan polymorphism, sistem dapat menampilkan informasi berbagai produk elektronik menggunakan satu method yang sama (getInfo()).**
